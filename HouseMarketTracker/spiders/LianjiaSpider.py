...
class LianjiaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for building in buildings:
            ...
            yield from ParseUtil.start_request(root_url + "xiangqing/", DetailParser().parse, meta)


class HouseLayoutParser():

    def parse(self, response):
        meta = response.meta
        item = meta['item']

        layout_li_s = response.xpath('//li[@class="huxing-item"]')
        layout_list = []
        for layout_li in layout_li_s:
            layout_dict = {}
            layout_dict['layout_type_name'] = layout_li.xpath('a/span/text()').extract_first()
            layout_dict['layout_image_url'] = layout_li.xpath('a/div/img/@src').extract_first()

            info_clear_div = layout_li.xpath('div')
            for row in info_clear_div.xpath('ul/li/text()').extract():
                res = re.search(r'(.+?)：(.+)', row)
                key = res.group(1)
                value = res.group(2)
                layout_dict[key] = value
            layout_dict['price'] = ''.join(info_clear_div.xpath('span[@class="price"]//text()').extract())
            layout_dict['sales_status'] = info_clear_div.xpath('span[@class="status on-sell"]/text()').extract_first()
            layout_list.append(layout_dict)
        item['house_layout'] = layout_list
...
class DetailParser():
    ...
    def parse_pre_sales(self, response):
        pre_sales_list = []
        tr_s = response.xpath('/html/body/div[4]/div[1]/table/tbody/tr')
        th_list = []
